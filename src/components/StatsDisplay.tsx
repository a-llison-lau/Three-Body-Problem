...
const StatsDisplay = ({ stats }: StatsDisplayProps) => {
  ...
  return (
    <div className="flex justify-center items-center w-full overflow-x-auto supports-backdrop-blur:bg-white/90 backdrop-blur-xl p-2 rounded-md">
      <style>
        {`
        ...
        <div className="space-y-2">
          {stats.velocities.map((velocity, i) => (
            <div key={i} className="text-lg flex items-baseline">
              <span
                className={`font-semibold math-italic mr-2 ${
                  i === 0
                    ? "text-red-500"
                    : i === 1
                    ? "text-green-500"
